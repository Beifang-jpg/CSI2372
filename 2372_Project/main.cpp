...
void sortAccounts(BankAccount ** list)
{
    BankAccount *temp;
    int accountNumber = 0;
    while(list[accountNumber]->getType() == 01 || list[accountNumber]->getType() == 02
          || list[accountNumber]->getType() == 03 || list[accountNumber]->getType() == 04){
        accountNumber ++;
    }
    ...
    for (int i = 0; i < accountNumber; i++){
        int j = 0;
        bool stop = false;
        while (list[i+j]->getAccountId() >= list[i+j+1]->getAccountId() && !stop){
            if (list[i+j+1]->getAccountId() != 0){
                temp = list[i+j+1] ;
                list[i+j+1] = list[i+j];
                list[i+j] = temp;
                j++;
...
//                                        char * name, long newDate, double newBalance):
        ...
        if (TypeRead == 01){
            ...
            listAccounts[counter] = new DepositAccount(accountRead, TypeRead, nameRead, dateRead, balanceRead, nbyearRead);

        }

        else if (TypeRead == 04){
            listAccounts[counter] = new LoanAccount(accountRead, TypeRead, nameRead, dateRead, balanceRead, nbyearRead, RateRead);
            BankAccount *const currentClients = listAccounts[counter];
            LoanAccount *loan = dynamic_cast<LoanAccount *>(currentClients);
            double b = loan->getBalance();
            double r = loan->getRate();
            double n = loan->getNbYears();
            loan->setBalance(b + (b* r * (n)/36000.00));
            //setBalance(getBalance() + (getBalance()* getRate() * (getNbYears())/36000.00));
...
void updateAccounts(BankAccount ** listAccounts) {
    ...
    long accountRead, dateRead;
    int TypeRead, transCode;
    double amountRead;

    int accountNumber = 0;
    while(listAccounts[accountNumber]->getType() == 01 || listAccounts[accountNumber]->getType() == 02
          || listAccounts[accountNumber]->getType() == 03 || listAccounts[accountNumber]->getType() == 04){
        accountNumber ++;
    }

    inputFile >> accountRead >> TypeRead >> dateRead >> transCode >> amountRead;
    inputFile.ignore(256, '\n');

    while(inputFile){

        for (int i = 0 ; i< accountNumber; i++){
            if (accountRead == listAccounts[i]->getAccountId() && TypeRead == listAccounts[i]->getType()){
                //Make sure the ID and type match
                // Doing deposit
                if (transCode == 01){
                    if (listAccounts[i]->getType() == 04){
                        listAccounts[i]->setBalance(listAccounts[i]->getBalance() - amountRead);

                        listAccounts[i]->setUpdateDate(dateRead);

                    }else if(listAccounts[i]->getType() == 01 || listAccounts[i]->getType() == 02 ||
                             listAccounts[i]->getType() == 03){
                        listAccounts[i]->setBalance(listAccounts[i]->getBalance() + amountRead);

                        listAccounts[i]->setUpdateDate(dateRead);
                    }
                }
                    //End of doing deposit
                    //Doing withdraw
                else if(transCode == 02){
                    if (listAccounts[i]->getType() == 01 || listAccounts[i]->getType() == 02 ){
                        listAccounts[i]->setBalance(listAccounts[i]->getBalance() - amountRead-0.5);

                        listAccounts[i]->setUpdateDate(dateRead);
                    }else{//Unable to do withdraw
                    }
                }
                    //End of doing withdraw
                    //Doing Checking
                else if(transCode == 03){
                    if(listAccounts[i]->getType() == 01){
                        if (listAccounts[i]->getType() == 01 ){
                            listAccounts[i]->setBalance(listAccounts[i]->getBalance() - amountRead);
...
void displayAccounts(BankAccount ** listAccounts)
{
    ...
    for (i = 0; i< accountNumber; i++){
        ...
        if (listAccounts[i]->getType() == 4){
            BankAccount *const currentClients = listAccounts[i];
            LoanAccount *loan = dynamic_cast<LoanAccount *>(currentClients);
            cout << "          ";
        ...
        else if (listAccounts[i]->getType() == 3 ){
            BankAccount *const currentClients = listAccounts[i];
            DepositAccount *depos = dynamic_cast<DepositAccount *>(currentClients);
            cout << "          ";
            depos->print();

        }
        else if (listAccounts[i]->getType() == 1 ||  listAccounts[i]->getType() == 2){
            cout << fixed;
            cout <<setprecision(2) << "          " << listAccounts[i]->getAccountId() << "                 " << listAccounts[i]->getType() << "       "<< listAccounts[i]->getUpdatedate() << "          "<< listAccounts[i]->getBalance()
                 << "      " ;
            cout <<  "" << endl;
        }
        totalAccounts += listAccounts[i]->getBalance();
        if (i != accountNumber-1){
            if (listAccounts[i]->getAccountId() != listAccounts[i+1]->getAccountId() ){
                cout << fixed;
                cout <<setprecision(2) <<  "                      TOTAL ACCOUNTS: " << totalAccounts << endl;
                cout << ""<<endl;
                totalAccounts = 0;
            }
        }
        else if (i == accountNumber-1){
            cout << fixed;
            cout <<setprecision(2) <<  "                      TOTAL ACCOUNTS: " << totalAccounts << endl;
            cout << ""<<endl;
            totalAccounts = 0;
...
int main()
{
    ...
    cout << "               ************************************************" << endl;
    cout << "               Alan Wang\n"
            "               Student Number: 300136722\n"
            "               Dengsong Wang\n"
            "               Student Number: 300136722" << endl;
